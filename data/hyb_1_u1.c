...
*              int n=653                  number of coefficients for input polynomials a(x) and b(x) 
**************************************************/
...
void ka2_real(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define KA2_REAL_MAX_AB_SIZE 13 // n/2
	#define KA2_REAL_MAX_P_SIZE 25 // n-1
...
void ka2_complex(struct complex a[], struct complex b[], struct complex c[], int n){
	
	#define KA2_COMPLEX_MAX_AB_SIZE 13 // n/2
	#define KA2_COMPLEX_MAX_P_SIZE 25 // n-1


    if(n == 13){
        sb_comba_complex(a, b, c, n);
        return;}
	...
	struct complex sum_a0_a1[KA2_COMPLEX_MAX_AB_SIZE];
	struct complex sum_b0_b1[KA2_COMPLEX_MAX_AB_SIZE];

	sum(a0, a1, sum_a0_a1, n2);
	sum(b0, b1, sum_b0_b1, n2);

	struct complex p1[KA2_COMPLEX_MAX_P_SIZE];
	struct complex p2[KA2_COMPLEX_MAX_P_SIZE];
	struct complex p3[KA2_COMPLEX_MAX_P_SIZE];
	memset(p1, 0, sizeof(p1));
	memset(p2, 0, sizeof(p2));
	memset(p3, 0, sizeof(p3));
	...
	ka2_complex(a0, b0, p1, n2);
	ka2_complex(sum_a0_a1, sum_b0_b1, p2, n2);
        ka2_complex(a1, b1, p3, n2);


	sum(c, p1, c, n-1);
	difference(c+n2, p3, c+n2, n-1);
	...
	sum(c+n2, p2, c+n2, n-1);
	

}


void ub_real_25(struct complex a[], struct complex b[], struct complex c[], int n){
	
	#define UB_REAL_MAX_AB_SIZE 13 // ceil(n/2)
	#define UB_REAL_MAX_P_SIZE 25 // n
...
void ub_real(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define UB_REAL_MAX_AB_SIZE 22 // ceil(n/2)
	#define UB_REAL_MAX_P_SIZE 43 // n
...
void ub_complex(struct complex a[], struct complex b[], struct complex c[], int n){
	
	#define UB_COMPLEX_MAX_AB_SIZE 12 // ceil(n/2)
	#define UB_COMPLEX_MAX_P_SIZE 23 // n
...
void a3_complex(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define A3_COMPLEX_MAX_AB_SIZE 9 // n/3
	#define A3_COMPLEX_MAX_P_SIZE 17 // 2*n/3-1
	...
	struct complex ra1[n3];
	struct complex ra2[n3];
	struct complex ra3[n3];
	struct complex ra4[n3];
	struct complex ra5[n3];
	struct complex rb1[n3];
	struct complex rb2[n3];
	struct complex rb3[n3];
	struct complex rb4[n3];
	struct complex rb5[n3];
	...
	struct complex p0[p_size];
	struct complex p1[p_size];
	struct complex p2[p_size];
	struct complex p3[p_size];
	struct complex p4[p_size];
	...
	struct complex u1[p_size];
	struct complex u2[p_size];
	struct complex u3[p_size];
	struct complex u4[p_size];
	struct complex u5[p_size];
	...
	struct complex c1[p_size];
	struct complex c2[p_size];
	struct complex c3[p_size];
...
void ue_v14_real(struct complex a[], struct complex b[], struct complex c[], int n){
   ...
   // UE_V14 for k=4
	#define UE_V1_REAL_MAX_AB_SIZE 27 // (n+4)/5    
	#define UE_V1_REAL_MAX_P_SIZE 53 // 2*(n+4)/5-1
	
	#define UE_V1_REAL_MAX_AB_SIZE_LOW 23 // (n-4*4)/5
	#define UE_V1_REAL_MAX_P_SIZE_LOW  45 // 2*(n-4*4)/5-1
	...
	sb_comba_real(a0, b0, p0, n5_high);
	sb_comba_real(ra11, rb11, p1, n5_high);
	a3_complex(ra12, rb12, p2, n5_high);
	phase_change(p2, p3, p_size);
	a3_complex(ra14, rb14, p4, n5_high);
	phase_change(p4, p5, p_size);
        a3_complex(ra16, rb16, p6, n5_high);
	phase_change(p6, p7, p_size);
	
	sb_comba_real(a4, b4, p8, n5_low);
...
void ue_v14_complex(struct complex a[], struct complex b[], struct complex c[], int n){

	#define UE_V1_COMPLEX_MAX_AB_SIZE 27 // (n+4)/5
	#define UE_V1_COMPLEX_MAX_P_SIZE 53 // 2*(n+4)/5-1
	
	#define UE_V1_COMPLEX_MAX_AB_SIZE_LOW 23 // (n-4*4)/5
	#define UE_V1_COMPLEX_MAX_P_SIZE_LOW  45 // 2*(n-4*4)/5-1
	...
	a3_complex(a0, b0, p0, n5_high);  
	a3_complex(ra11, rb11, p1, n5_high);
	a3_complex(ra12, rb12, p2, n5_high);
	a3_complex(ra13, rb13, p3, n5_high);
	a3_complex(ra14, rb14, p4, n5_high);
	a3_complex(ra15, rb15, p5, n5_high);
	a3_complex(ra16, rb16, p6, n5_high);
	a3_complex(ra17, rb17, p7, n5_high);
	
	ub_complex(a4, b4, p8, n5_low);
	...
	sum(c+8*n5_high, c8, c+8*n5_high, p_size-8);
...
void ue_v11_real(struct complex a[], struct complex b[], struct complex c[], int n){
   ...
   // UE_V11 for k=1
	#define UE_V1_REAL_MAX_AB_SIZE  26 // (n+1)/5    
	#define UE_V1_REAL_MAX_P_SIZE  51 // 2*(n+1)/5-1
	
	#define UE_V1_REAL_MAX_AB_SIZE_LOW 25 // (n-4*1)/5
	#define UE_V1_REAL_MAX_P_SIZE_LOW  49 // 2*(n-4*1)/5-1
	...
	ka2_complex(ra12, rb12, p2, n5_high);
	phase_change(p2, p3, p_size);
	ka2_complex(ra14, rb14, p4, n5_high);
	phase_change(p4, p5, p_size);
        ka2_complex(ra16, rb16, p6, n5_high);
	phase_change(p6, p7, p_size);
	
	ub_real_25(a4, b4, p8, n5_low);
...
void ue_v12_real(struct complex a[], struct complex b[], struct complex c[], int n){
   ...
   // UE_V12 for k=2
	#define UE_V1_REAL_MAX_AB_SIZE 131 // (n+2)/5    
	#define UE_V1_REAL_MAX_P_SIZE 261 // 2*(n+2)/5-1
	
	#define UE_V1_REAL_MAX_AB_SIZE_LOW 129 // (n-4*2)/5
	#define UE_V1_REAL_MAX_P_SIZE_LOW  257 // 2*(n-4*2)/5-1
	/* Assumed n = 5^t-3 for some non-negative integer t */
	int n5_high = (n+2)/5;
	int n5_low = (n-8)/5; 
	int p_size = 2*n5_high-1;
	...
	ue_v14_real(a0, b0, p0, n5_high);
	ue_v14_real(ra11, rb11, p1, n5_high);
	ue_v14_complex(ra12, rb12, p2, n5_high);
	phase_change(p2, p3, p_size);
	ue_v14_complex(ra14, rb14, p4, n5_high);
	phase_change(p4, p5, p_size);
        ue_v14_complex(ra16, rb16, p6, n5_high);
	phase_change(p6, p7, p_size);
...
int main(){
	...
	struct complex *a = malloc(653*sizeof(struct complex));
	struct complex *b = malloc(653*sizeof(struct complex));
	struct complex *c = calloc((2*653-1), sizeof(struct complex));

       int i;
       
//reading a and b from the file inp653test
     FILE *myFile;
    myFile = fopen("inp653test", "r");
    if (myFile == NULL){
    ...
    fclose(myFile);
    	...
    	clock_t start = clock ();	     
   	for (i=0;i<99999;i++){
           ue_v12_real(a, b, c, 653);
           memset(c, 0, sizeof(struct complex)*(2*653-1));
    	   }
    	ue_v12_real(a, b, c, 653);
 ...
 //writing on c_ue_v1_653 file	
	FILE *outFile;
    outFile = fopen("c_ue_v1_653", "w");
    if (outFile == NULL)
	{
