...
module tb;
    ...
    initial begin
        ...
        for(i=0;i<761;i=i+1) begin r=$fscanf(fd,"%d",v); waitnen[i]=v[1:0]; end
        $fclose(fd);
        rst=1; start=0; repeat(2) @(negedge clk); rst=0; @(negedge clk);
        start=1; @(negedge clk); start=0;
        wait(done==1); #1;
        $display("cycle_count=%0d", cycle_count);
        for(i=0;i<761;i=i+1) if(cr[2*i+:2]!==waitnen[i]) begin
            if(error<5) $display("FAIL c[%0d]: v=%0d bek=%0d",i,cr[2*i+:2],waitnen[i]);
            error=error+1; end
        if(error==0) $display("=== b1_r3_761_serial PASS (761/761 == ref R/3, %0d cycle) ===",cycle_count);
        else $display("=== %0d HATA ===",error);
        $finish;
    end
