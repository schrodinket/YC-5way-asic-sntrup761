...
module tb;
    ...
    integer i,v,error,r,fd;
    reg [1:0] waitnen [0:1534];
    ...
    initial begin
        ...
        for(i=0;i<1535;i=i+1) begin r=$fscanf(fd,"%d",v); waitnen[i]=v[1:0]; end
        $fclose(fd);
        rst=1;start=0;repeat(2)@(negedge clk);rst=0;@(negedge clk);start=1;@(negedge clk);start=0;
        wait(done==1);#1;
        for(i=0;i<1535;i=i+1) if(cr[2*i+:2]!==waitnen[i]) begin
            if(error<5)$display("HAM FAIL[%0d]: v=%0d bek=%0d",i,cr[2*i+:2],waitnen[i]); error=error+1; end
        if(error==0)$display("=== HAM PASS (1535/1535) - b1_real_serial dogru ===");
        else $display("=== HAM %0d HATA - sorun b1_real_serial'de ===",error);
        $finish;
    end
