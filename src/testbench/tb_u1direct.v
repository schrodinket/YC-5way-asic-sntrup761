...
// u1_r3_1motor ciktisi = benim ref hesabim (mod-3, indirgemeli) ile karsilastir
module tb_u1direct;
    ...
    u1_r3_1motor dut(.clk(clk),.rst(rst),.start(start),.ar(ar),.br(br),
        .cr(cr),.done(done),.cycle_count(cc));

    integer ea[0:P-1], ga[0:P-1], ref_c[0:P-1];
    integer i,j,k;
    initial begin
        // {0,1,2} girdi (C native kodlama, -1 yok)
        ...
        // ref: konvolusyon + x^761=x+1 indirgeme, mod-3 {0,1,2}
        begin: rc
        ...
        begin: cmp
            ...
            $display("U1 DOGRUDAN (remap yok): %0d/%0d hata -> %s",err,P,(err==0)?"PASS":"FAIL");
        end
        $finish;
    end
endmodule
