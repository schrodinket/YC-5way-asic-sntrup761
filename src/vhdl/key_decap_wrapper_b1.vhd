...
entity key_decap_wrapper_b1 is
	port(
		clock                : in  std_logic;
		reset                : in  std_logic;
...
end entity key_decap_wrapper_b1;

architecture RTL of key_decap_wrapper_b1 is
	...
	signal reencap_r3_latch : std_logic;
...
begin
	...
	reencap_r3_latch <= '0' when state_dec_wrap /= DECAP_CORE_R3 else '1' when (key_encap_small_weights_valid = '1' or key_decap_done = '1') else reencap_r3_latch when rising_edge(clock);
	...
	bram_r_enc_write_a   <= encode_Zx_output_valid when state_dec_wrap = DECAP_CORE_WAIT or state_dec_wrap = DECAP_CORE_R3 else '0';

	k_hash_out(63 downto 0) <= sha_out;

	key_decapsulation_inst : entity work.key_decapsulation_b1
		port map(
			clock                => clock,
			reset                => reset,
	...
	reencap_true <= '1' when state_dec_wrap = REENCAP or (state_dec_wrap = DECAP_CORE_WAIT and key_decap_done = '1') or (state_dec_wrap = DECAP_CORE_WAIT and counter_pipe /= 0) or (state_dec_wrap = DECAP_CORE_R3 and (key_encap_small_weights_valid = '1' or key_decap_done = '1' or reencap_r3_latch = '1')) else '0';  -- cover both load_small and start windows; e preserved
