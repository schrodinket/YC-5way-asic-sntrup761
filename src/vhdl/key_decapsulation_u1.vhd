...
entity key_decapsulation_u1 is
	port(
		clock                : in  std_logic;
		reset                : in  std_logic;
		start                : in  std_logic;
...
end entity key_decapsulation_u1;

architecture RTL of key_decapsulation_u1 is
	...
	-- U1 multiplier interface signals
	...
	signal u1_start     : std_logic;
	signal u1_done      : std_logic;
	signal u1_ov        : std_logic;
	signal u1_output    : std_logic_vector(1 downto 0);
	signal r3_started   : std_logic;  -- single-cycle start pulse
	signal u1_result_wide : std_logic_vector(2*p - 1 downto 0);  -- U1 result, 2*p bits
	signal u1_result_ready: std_logic;
...
begin
	...
	begin
		if reset = '1' then
			...
			case state_decap is
				...
				when mult_R3 =>
					-- start parallel weight once U1 result is ready
					r3_mult_start     <= '0';
					if u1_result_ready = '1' then
	...
	-- output served from U1 register
	-- guard against overflow at counter = p
	ev_raw     <= u1_result_wide(2*counter + 1 downto 2*counter) when (state_decap = output_masked_weight and counter < p) else (others => '0');
	ev     <= "11" when ev_raw = "10" else ev_raw;  -- map U1 encoding (-1=10) to Peng encoding (-1=11)
	...
	r3_mult_done        <= u1_done;  -- bridge done
	...
	r3_mult_output_valid <= u1_ov when state_decap = mult_R3 or state_decap = calc_weight else '0';
	r3_mult_output       <= u1_output;
	...
	to_rq_mult.start        <= rq_mult_start when state_decap = idle or state_decap = mult_Rq else '0';  -- do not start rq_mult during mult_R3; U1 handles it
	to_rq_mult.output_ack   <= '1';
	to_rq_mult.load_small_g <= rq_mult_load_small when state_decap = idle or state_decap = mult_Rq else '0';  -- do not load_small during mult_freeze/R3; U1 handles R/3
	...
	-- U1 R/3 bridge instance
	-- u1_start pulses for one cycle on entering mult_R3
	u1_start <= r3_mult_start;

	r3_u1_bridge_inst : r3_u1_bridge
		...
		port map (
			...
			output_valid => u1_ov,
			r3_output    => u1_output,
			result_wide  => u1_result_wide,
			result_ready => u1_result_ready
		);
